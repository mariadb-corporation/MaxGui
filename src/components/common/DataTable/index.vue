        ...
        <tr v-if="windowSize.x >= 600">
          ...
          >
            <div class="d-inline-flex justify-center align-center">
              ...
              <v-icon v-if="header.sortable !== false" size="7" class="ml-1 v-data-table-header__icon">$vuetify.icons.arrowDown</v-icon>
            </div>
          </th>
          ...
          <th v-if="windowSize.x > 960" style="padding-left:24px; padding-right:0px">
            <div v-if="hasSearch || hasColumnToggle" style="width:100%;" class="d-flex align-center ">
              <search-box
              ...
              <v-icon v-if="hasColumnToggle" size="18" class="pl-4 pr-0 toggle-icon" @click.stop="columnToggle">
                visibility
              </v-icon>
            </div>
          </th>
    ...
    <template v-slot:item="{ item, index }">
      ...
      >
        <td
          ...
          :key="i"
          :class="[header.value, header.tdClass || header.class, windowSize.x < 600 && 'v-data-table__mobile-row']"
              ...
              <div v-if="header.text === 'Actions'" :class="`d-flex justify-${header.align}`">
                ...
                <v-tooltip top>
                  <template v-slot:activator="{ on }">
                    <v-btn v-if="$scopedSlots['expandable']" icon color="primary" v-on="on" @click="toggleRow(item.id)">
                      <!-- optional expand indicator icon -->
                      <slot name="expandIndicator" :expanded="expandedRows.includes(item.id)">
                        <v-icon v-if="!expandedRows.includes(item.id)" size="24">keyboard_arrow_down</v-icon>
...
export default {
  ...
  methods: {
    ...
    getValue(item, header) {
      const value = item[header.value] || item[header.value] === 0 ? item[header.value].text || item[header.value] : 'n/a';
