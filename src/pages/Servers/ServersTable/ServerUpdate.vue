  ...
  <span>
    ...
    <base-dialog v-model="dialog" :onCancel="() => (dialog = false)" :onSave="handleUpdate" maxWidth="800px">
        ...
        <v-card-text>
          <v-container>
            <v-form ref="form" v-model="isValid" @keyup.native.enter="isValid && handleUpdate()">
              <v-row>
                <v-col xs="12" sm="6">
                  <v-text-field id="id" v-model="item.id" label="Id of the server*" name="id" required disabled />
                </v-col>
              </v-row>
              ...
              <v-row>
                ...
                <v-col xs="12" sm="6">
                  ...
                  <div v-if="relationships.services.data.length" class="scrollable-input-div">
                    <div v-for="(service, i) in relationships.services.data" :key="i">
                      <v-btn class="delete" right icon x-small @click="deleteRelationship('services', service.id)">
                ...
                <v-col xs="12" sm="6">
                  <v-btn v-if="relationships.monitors.data.length == 0" color="primary" x-small @click="addRelationship('monitors')">
                    Add Monitor
                  </v-btn>
                  <div v-else style="height:20px" />
                  <div v-if="relationships.monitors.data.length" class="scrollable-input-div">
                    <div v-for="(monitor, i) in relationships.monitors.data" :key="i">
                      <v-btn class="delete" icon x-small @click="deleteRelationship('monitors', monitor.id)">
                        <v-icon color="red" size="16">close</v-icon>
                      </v-btn>
                      <v-text-field
                        id="monitor_id"
      ...
      <template v-slot:actions="{ cancel, save }">
        ...
        <v-btn color="red" :disabled="!isValid" text depressed @click="save">
          Update
        </v-btn>
      </template>
    </base-dialog>
