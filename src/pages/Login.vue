        ...
        <v-card class="v-card-custom" style="max-width: 463px; z-index: 2;border-radius: 10px;">
          <v-card-text style="padding-top:60px" class="pb-0" align-center>
            <div class="px-12">
              ...
              <v-form ref="form" v-model="isValid" class="pt-4" @keyup.native.enter="isValid && handleSubmit()">
                <v-text-field
                  id="username"
                ...
                <v-checkbox v-model="rememberMe" class="small mt-2" label="Remember Me" color="primary" />
              </v-form>
            </div>
          </v-card-text>
          <v-card-actions style="padding-bottom:60px" class="pt-0">
            <div class="mx-auto text-center" style="width: 50%;">
              ...
              <v-btn v-else class="mx-auto login" :disabled="!isValid" block depressed color="primary" @click="handleSubmit">
                <span class="font-weight-regular text-capitalize">Sign In</span>
              </v-btn>
              <a href style="font-size:0.865rem;text-decoration: none;" class="d-block mx-auto mt-2">Forgot password</a>
            </div>
          </v-card-actions>
        </v-card>
      </v-col>
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        if (error.response) {
          this.errorMessage = error.response.status === 401 ? 'Incorrect password or username' : this.$help.getErrorsArr(error);
        }
      }
